...
function OpenAirShop(shop)
	...
	local elements  = {
		{unselectable = true, icon = "fas fa-ship", title = TranslateCap('air_shop')}
	}
					...
					ESX.OpenContext("right", elements3, function(menu3,element3)
						if element3.value == "buy" then
							...
							ESX.TriggerServerCallback('esx_air:buyAir', function(bought)
								if bought then
									ESX.ShowNotification(TranslateCap('air_shop_bought', element.name, ESX.Math.GroupDigits(element.price)))
									...
									CurrentActionMsg = TranslateCap('air_shop_open')

									FreezeEntityPosition(playerPed, false)
									SetEntityVisible(playerPed, true)
									SetEntityCoords(playerPed, shop.Outside.x, shop.Outside.y, shop.Outside.z)
								else
									ESX.ShowNotification(TranslateCap('air_shop_nomoney'))
								end
							end, props)
...
function OpenAirGarage(garage)
	ESX.TriggerServerCallback('esx_air:getGarage', function (ownedAirs)
		if #ownedAirs == 0 then
			ESX.ShowNotification(TranslateCap('garage_noairs'))
		else
			...
			for i=1, #ownedAirs, 1 do
				...
				elements[#elements+1] = {
					icon = "fas fa-ship",
					title = getVehicleLabelFromHash(ownedAirs[i].model),
...
function OpenLicenceMenu(shop)
	local elements = {
		...
		{icon = "fas fa-ship", title = "Vásároljon repülő engedélyt"}
	}
